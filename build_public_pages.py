...
def build_index():
    ...
    # Quick nav links
    links = [('About Us','about.html'),('Our Services','services.html'),('Testimonials','testimonials.html'),
             ('FAQs','faqs.html'),('Help Center','help.html'),('Awards','awards.html'),
             ('Contact Us','contact.html'),('Browse All Schema Files','#files')]
    quick_links = ''.join(f'<li style="margin:0.5rem 0;"><a href="{url}" style="font-size:1.1em;font-weight:500;">{esc(name)}</a></li>' for name, url in links)
    ...
    for d in schema_dirs:
        ...
        for root, dirs, files in os.walk(d):
            for fname in sorted(files):
                if fname.endswith(('.json','.html','.md','.yaml','.yml')):
                    filepath = os.path.join(root, fname).replace('\\','/')
                    if base_url:
                        full_url = f'{base_url}/{filepath}'
                        file_links.append(f'<li><a href="{full_url}" target="_blank">{esc(filepath)}</a></li>')
                    else:
                        file_links.append(f'<li><a href="{filepath}">{esc(filepath)}</a></li>')

    # Stats
    dirs_labels = [('faqs','FAQs'),('help','Help Articles'),('services','Services'),('webpages','Webpages'),
                   ('awards','Awards'),('case-studies','Case Studies'),('locations','Locations'),
                   ('organization','Organization'),('press','Press'),('products','Products'),('reviews','Reviews'),('team','Team')]
    stats = [(label, count_files(d)) for d, label in dirs_labels]
    stats = [(l, c) for l, c in stats if c > 0]
...
<ul>{''.join(f'<li><strong>{c}</strong> {l}</li>' for l, c in stats)}</ul>
<p><strong>Total schemas:</strong> {total}</p>
<h2 id="files">\U0001f4c1 All Schema Files</h2>
<ul>{''.join(sorted(file_links))}</ul>
"""
    write_page('index.html', f'Welcome to {BIZ}', content, f'{BIZ}: {total} structured data schemas for AI visibility.')
...
<a href="#{slug}">\U0001f517 Permalink</a>
    ...
    content = ''.join(cards) if cards else '<p>No services listed yet.</p>'
...
<div style="margin-top:0.5rem;">{stars}</div>
    ...
    content = ''.join(cards) if cards else '<div class="card"><p>No testimonials have been published yet. Check back soon.</p></div>'
...
def build_awards():
    ...
    content = ''.join(cards) if cards else '<p>No awards found yet.</p>'
